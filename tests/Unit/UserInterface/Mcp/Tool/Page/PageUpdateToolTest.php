...
final class PageUpdateToolTest extends TestCase
{
    ...
    public function testUpdatePageSetsTheShadow(): void
    {
        $this->setUpReadModifyWrite('uuid-1', 'en', ['template' => 'default', 'title' => 'Existing']);
        ...
        $this->tool->updatePage('uuid-1', 'en', shadowOn: true, shadowLocale: 'de');
    ...
    public function testUpdatePageClearsTheShadow(): void
    {
        $this->setUpReadModifyWrite('uuid-1', 'en', ['template' => 'default', 'title' => 'Existing']);
        ...
        $this->tool->updatePage('uuid-1', 'en', shadowOn: false);

        $data = $capturedEnvelope->getMessage()->getData();
        $this->assertFalse($data['shadowOn']);
        $this->assertNull($data['shadowLocale']);
    }

    public function testUpdatePageLeavesTheShadowUntouchedWhenOmitted(): void
    {
        $this->setUpReadModifyWrite('uuid-1', 'en', ['template' => 'default', 'title' => 'Existing']);
        ...
        $data = $capturedEnvelope->getMessage()->getData();
        $this->assertArrayNotHasKey('shadowOn', $data);
        $this->assertArrayNotHasKey('shadowLocale', $data);
    }

    public function testUpdatePageRejectsAShadowWithoutALocale(): void
    {
        $this->setUpReadModifyWrite('uuid-1', 'en', ['template' => 'default', 'title' => 'Existing']);
        $this->messageBus->dispatch(Argument::cetera())->shouldNotBeCalled();

        $result = $this->tool->updatePage('uuid-1', 'en', shadowOn: true);

        $this->assertArrayHasKey('error', $result);
        $this->assertStringContainsString('shadowLocale', $result['error']);
    }

    public function testUpdatePageRejectsALocaleShadowingItself(): void
    {
        $this->setUpReadModifyWrite('uuid-1', 'en', ['template' => 'default', 'title' => 'Existing']);
        $this->messageBus->dispatch(Argument::cetera())->shouldNotBeCalled();

        $result = $this->tool->updatePage('uuid-1', 'en', shadowOn: true, shadowLocale: 'en');
    ...
    public function testUpdatePageRejectsAShadowOnALinkedPage(): void
    {
        ...
        $result = $this->tool->updatePage('uuid-1', 'en', shadowOn: true, shadowLocale: 'de');

        $this->assertArrayHasKey('error', $result);
        $this->assertStringContainsString('link', $result['error']);
    }

    public function testUpdatePageRejectsShadowingALocaleThatIsAlreadyMirrored(): void
    {
        $this->setUpReadModifyWrite('uuid-1', 'en', [
            'template' => 'default',
            'title' => 'Existing',
